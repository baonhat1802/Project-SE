...
export default function OrderScreen() {
  ...
  const updateStock_ = async (item) => {
    ...
    await axios.put(`/api/products/${item._id}`,
      { 
        ...
        brand: data.brand,
        countInStock : data.countInStock - item.quantity,
  ...
  function updateStock(order){
    order.orderItems.map((item) => (updateStock_(item)))
  };
          ...
          <Card className="mb-3">
            <Card.Body>
              ...
              <ListGroup variant="flush">
                ...
                {(userInfo.isAdmin || userInfo.isEmployee) && order.isPaid && !order.isDelivered && (
                  <ListGroup.Item>
